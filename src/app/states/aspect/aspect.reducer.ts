...
import { AspectState } from './aspect.state';
import {
...
import { MrsnvCandidate } from '../../data/schema/mrsnv-candidate';
import { MrsnvNetworkAttributes } from '../../data/schema/mrsnv-network-attributes';
import { ValidationErrorLengths } from '../../data/schema/validation-error-lengths';
...
const initialState = createFormGroupState<AspectState>('mrsnv', {
  editable: false,
...
const rawMrsnvReducer = createReducer(
  ...
  on(
    checkNetworkCompatibilitySuccess,
    checkNetworkCompatibilityUuidSuccess,
    (state: FormGroupState<AspectState>, { mrsnv }): FormGroupState<AspectState> => {
      ...
      const newState = createFormGroupState<AspectState>(initialState.id, stateValue);
      return disable(newState);
    },
  ),
  on(checkNetworkCompatibility, checkNetworkCompatibilityUuid, (): FormGroupState<AspectState> => {
    return disable(createFormGroupState<AspectState>(initialState.id, initialState.value));
  }),
  on(createOrEditMrsnvAspect, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const newState = createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
      editable: true,
      canExitView: false,
    });
  ...
  on(loadNetworkRaw, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const newState = createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
  ...
  on(
    loadNetworkRawSuccess,
    (state: FormGroupState<AspectState>, { network }): FormGroupState<AspectState> => {
      // NETWORK ATTRIBUTES
  ...
  on(loadNetworkRawFailure, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const newState = createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
      loadingSelectedNetwork: false,
    });
  ...
  on(addInfo, (state: FormGroupState<AspectState>, { infoName }): FormGroupState<AspectState> => {
    if (!state.value.candidatesNetworkAttributes) {
      return createFormGroupState<AspectState>(initialState.id, state.value);
    }
    ...
    return createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
      networkAttributes: {
  ...
  on(removeInfo, (state: FormGroupState<AspectState>, { infoName }): FormGroupState<AspectState> => {
  ...
  on(setDefaultSeparator, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
  ...
  on(setCustomSeparator, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    return createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
      separator: '',
      savedSeparator: '',
  ...
  on(enableSeparatorInput, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    return enable(state);
  }),
  on(submitCustomSeparator, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
  ...
  on(resetCustomSeparator, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const newState = createFormGroupState<AspectState>(initialState.id, {
      ...state.value,
  ...
  onNgrxFormsAction(
    SetValueAction,
    (
      state: FormGroupState<AspectState>,
      action: SetValueAction<any>,
    ): FormGroupState<AspectState> => {
      ...
      // Re-instate the previous separator states
      return updateRecursive(newState, (s) => {
  ...
  on(
    displayGeneralPropertyCandidate,
    (state: FormGroupState<AspectState>, { candidate }): FormGroupState<AspectState> => {
      const newState = createFormGroupState<AspectState>(initialState.id, {
        ...state.value,
  ...
  on(addGeneralProperty, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const { value } = state;
  ...
  on(addIndividualProperty, (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
    const { value } = state;
  ...
  on(
    addMappingStepGeneral,
    (state: FormGroupState<AspectState>, { propertyIndex }): FormGroupState<AspectState> => {
  ...
  on(
    removeMappingStepGeneral,
    (state: FormGroupState<AspectState>, { index }): FormGroupState<AspectState> => {
      const pMappingsKeys = [...state.value.pMappingsKeys];
      const pMappingsValues = [...state.value.pMappingsValues];
  ...
  on(
    ...
    (state: FormGroupState<AspectState>, { result }): FormGroupState<AspectState> => {
      const uuid = result.split('/');
      if (uuid.length === 0) return disable(state);
      return disable(
        createFormGroupState<AspectState>(state.id, {
          ...state.value,
          editable: false,
  ...
  on(abortMrsnvEditing, navigateHome, (): FormGroupState<AspectState> => {
    // Hard reset
    const newState = createFormGroupState<AspectState>(initialState.id, {
      ...initialState.value,
      canExitView: true,
  ...
  on(
    ...
    (state: FormGroupState<AspectState>): FormGroupState<AspectState> => {
      const newState = createFormGroupState<AspectState>(initialState.id, {
        ...state.value,
        canExitView: true,
      });
...
const validateForm = updateGroup<AspectState>({
...
export const aspectReducer = wrapReducerWithFormStateUpdate(
  rawMrsnvReducer,
  (state: FormGroupState<AspectState>) => state,
  validateForm,
);
