    ...
    Object.entries(sorted).forEach(([rawKey, value], index) => {
      ...
      if (index !== keys.length - 1) {
        ...
        this.cyCore
          ...
          .style('background-color', value as string)
          .style('text-outline-color', value as string);
      }
    });
  }
    ...
    Object.entries(sorted).forEach(([rawKey, value], index) => {
      ...
      if (index !== keys.length - 1) {
        ...
        this.cyCore
          ...
          // @ts-ignore
          .selector(`node.split[colorA>${key}]`)
          .style('pie-2-background-color', value as string)
          // @ts-ignore
          .selector(`node.split[colorB>${key}]`)
          .style('pie-1-background-color', value as string)
          // @ts-ignore
          .selector('node.split[colorA][^colorB]')
          .style('pie-1-background-color', this.colors.gray)
          // @ts-ignore
