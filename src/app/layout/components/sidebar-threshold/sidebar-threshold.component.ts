...
} from '../../../states/threshold/threshold.selectors';
import {
  selectGroupLabelA,
  selectGroupLabelB,
...
} from '../../../states/threshold/threshold.action';
...
export class SidebarThresholdComponent implements OnInit {
  ...
  resetLowerBound(threshold: ThresholdDefinition) {
    this.store.dispatch(resetThresholdMin({ threshold }));
  }

  resetUpperBound(threshold: ThresholdDefinition) {
    this.store.dispatch(resetThresholdMax({ threshold }));
  }
}
